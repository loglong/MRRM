...
describe('RegisterDtoSchema', () => {
    ...
    describe('valid inputs', () => {
        it('should pass with all valid fields', () => {
            const result = register_dto_1.RegisterDtoSchema.safeParse(validDto);
            expect(result.success).toBe(true);
        });
        it('should pass without optional phone and orgId', () => {
            const { phone, orgId, ...required } = validDto;
            const result = register_dto_1.RegisterDtoSchema.safeParse(required);
            expect(result.success).toBe(true);
        });
    });
    describe('password validation', () => {
        it('should fail when password is less than 8 characters', () => {
            const result = register_dto_1.RegisterDtoSchema.safeParse({ ...validDto, password: 'Pass1' });
        ...
        it('should fail when password has no uppercase letter', () => {
            const result = register_dto_1.RegisterDtoSchema.safeParse({ ...validDto, password: 'password123' });
            expect(result.success).toBe(false);
            if (!result.success) {
        ...
        it('should fail when password has no number', () => {
            const result = register_dto_1.RegisterDtoSchema.safeParse({ ...validDto, password: 'PasswordABC' });
            expect(result.success).toBe(false);
            if (!result.success) {
    ...
    describe('other field validation', () => {
        it('should fail when name is empty', () => {
            const result = register_dto_1.RegisterDtoSchema.safeParse({ ...validDto, name: '' });
            expect(result.success).toBe(false);
            if (!result.success) {
        ...
        it('should fail when email format is invalid', () => {
            const result = register_dto_1.RegisterDtoSchema.safeParse({ ...validDto, email: 'not-an-email' });
            expect(result.success).toBe(false);
            if (!result.success) {
...
//# sourceMappingURL=register.dto.spec.js.map